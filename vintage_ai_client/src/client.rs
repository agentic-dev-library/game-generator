...
impl std::fmt::Display for AudioType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AudioType::ThemeSong => write!(f, "ThemeSong"),
            AudioType::BattleMusic => write!(f, "BattleMusic"),
            AudioType::VictoryFanfare => write!(f, "VictoryFanfare"),
            AudioType::SoundEffect(name) => write!(f, "SoundEffect_{}", name),
        }
    }
}
    ...
    pub async fn execute(&self, task: AiTask) -> Result<AiResult> {
        ...
        let (result, request_type, tokens, cost, cache_hit) = match task {
            ...
            AiTask::GenerateAudio {
                ...
                let cache_key = format!("audio_{}_{}_{}", game_name, audio_type, mood);
